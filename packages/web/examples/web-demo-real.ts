/**
 * demo:web:real（post-MVP 增补）：浏览器里的对话用**真模型**。
 *
 * 与 demo:web 共用同一个 runtime（web-demo-shared.ts），唯一区别是
 ...
 *
 * 用法：pnpm demo:web:real [--port 8081] [--static apps/web/dist] [--dir <目录>] [--clean]
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(`静态文件目录不存在：${staticDir}\n请先运行 pnpm build:web（demo:web:real 会先构建，直接跑 pnpm demo:web:real 即可）`)
    process.exit(1)
  }
