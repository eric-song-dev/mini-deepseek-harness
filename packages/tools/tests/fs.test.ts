...
/**
 * 文件工具契约（M3 spec 任务 3；2026-08-18 起名字/参数对齐上游工具目录：
 * read/write/edit + file_path/old_string/new_string）：
...
describe('文件工具', () => {
  ...
  it('read：读回文件内容；相对路径按会话 cwd 解析', async () => {
    ...
    await expect(read.execute({ file_path: 'a.txt' }, { cwd: dir })).resolves.toBe('文件内容')
  })

  it('read：绝对路径不叠加 cwd', async () => {
    const path = resolve(dir, 'b.txt')
    await writeFile(path, '绝对路径内容', 'utf8')
    await expect(read.execute({ file_path: path }, { cwd: '/somewhere-else' })).resolves.toBe('绝对路径内容')
  })

  it('read：文件不存在报错（模型需要看到失败原因）', async () => {
    await expect(read.execute({ file_path: 'missing.txt' }, { cwd: dir })).rejects.toThrow()
  })

  it('write：写入后读回一致（往返），返回实际路径与字节数', async () => {
    const result = (await write.execute({ file_path: 'sub/dir/a.txt', content: '新内容' }, { cwd: dir })) as {
      path: string
      bytes: number
    }
  ...
  it('write：父目录不存在自动创建；覆盖已有文件', async () => {
    ...
    await write.execute({ file_path: 'deep/nested/x.txt', content: '第一版' }, { cwd: dir })
    await write.execute({ file_path: 'deep/nested/x.txt', content: '第二版' }, { cwd: dir })
  ...
  it('edit：旧文本恰好出现一次时精确替换，返回替换成功', async () => {
    ...
    const result = (await edit.execute(
      { file_path: 'notes.md', old_string: '很好', new_string: '一般' },
      { cwd: dir },
    )) as {
      path: string
      replaced: boolean
  ...
  it('edit：旧文本不存在报错，文件原样不动', async () => {
    ...
    await expect(
      edit.execute({ file_path: 'notes.md', old_string: '不存在的文本', new_string: 'x' }, { cwd: dir }),
  ...
  it('edit：旧文本出现多次报错（精确替换 = 恰好一次），文件原样不动', async () => {
    ...
    await expect(
      edit.execute({ file_path: 'notes.md', old_string: 'aa', new_string: 'cc' }, { cwd: dir }),
    ).rejects.toThrow(/出现 2 次/)
    await expect(readFile(path, 'utf8')).resolves.toBe('aa bb aa')
  })

  it('edit：new_string 含 $ 特殊序列时按字面替换（$&/$$/$`/$\' 不解释为替换模式）', async () => {
    const path = resolve(dir, 'price.md')
    await writeFile(path, 'a: 10\nb: alpha\nc: beta\nd: gamma\n', 'utf8')

    // $& 在替换串里是"匹配到的文本"：字面替换必须写进文件的是两个字符 "$&"
    await edit.execute({ file_path: 'price.md', old_string: '10', new_string: '$&' }, { cwd: dir })
    await expect(readFile(path, 'utf8')).resolves.toContain('a: $&\n')

    // $$ 会被解释成"一个 $"：字面替换必须留下两个 $
    await edit.execute({ file_path: 'price.md', old_string: 'alpha', new_string: '$$' }, { cwd: dir })
    await expect(readFile(path, 'utf8')).resolves.toContain('b: $$\n')

    // $` 是"匹配位置之前的文本"：字面替换必须原样写入
    await edit.execute({ file_path: 'price.md', old_string: 'beta', new_string: '$`' }, { cwd: dir })
    await expect(readFile(path, 'utf8')).resolves.toContain('c: $`\n')

    // $' 是"匹配位置之后的文本"：字面替换必须原样写入
    await edit.execute({ file_path: 'price.md', old_string: 'gamma', new_string: "$\'" }, { cwd: dir })
    await expect(readFile(path, 'utf8')).resolves.toContain("d: $'\n")
  })

  it('read/write/edit 工具插件把三个声明注册进 tools 服务（上游同款名字与 file_path 参数）', async () => {
    ...
    try {
      ...
      expect(tools.list().map((d) => d.name)).toEqual(['read', 'write', 'edit'])
      for (const declaration of tools.list()) {
        expect(declaration.parameters).toEqual(
          expect.objectContaining({
            type: 'object',
            properties: expect.objectContaining({ file_path: expect.objectContaining({ type: 'string' }) }),
          }),
        )
      }
    } finally {
